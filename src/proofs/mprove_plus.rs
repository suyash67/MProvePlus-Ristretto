...
impl Constraints{
    pub fn generate_constraints(
        ...
        let ones_kronecker_yn = (0..sn)
            .map(|i| {
                let j = i % n;
                BigInt::mod_pow(&y, &BigInt::from(j as u32), &order)
            })
            .collect::<Vec<BigInt>>();
        ...
        let range = BigInt::mod_pow(&BigInt::from(5), &BigInt::from(4), &order);
        let q_int = BigInt::sample_below(&range);
        ...
        let v4 = (0..t)
            .map(|i| {
                if i >= (n+3) && i < (2*n+3){
                    minus_y_n[i-(n+3)].clone()
                }
                else if i >= (t-sn){
                    ones_kronecker_yn[i-(t-sn)].clone()
                }
                else {
                    BigInt::zero()
                }
        ...
        let v5 = (0..t)
            .map(|i| {
                if i == 1 {
                    let minus_y_pow_s = BigInt::mod_sub(&BigInt::zero(), &y_pow_s, &order);
                    minus_y_pow_s
                }
                else if i >= (t-sn){
                    ys_kronecker_1n[i-(t-sn)].clone()
                }
                else {
                    BigInt::zero()
                }
        ...
        let theta = (0..t)
            .map(|i| {
                let z_v1_i = BigInt::mod_mul(&z, &v1[i], &order);
                BigInt::mod_add(&z_v1_i, &v0[i], &order)
            })  
            .collect::<Vec<BigInt>>();
        ...
        let zeta = (0..t)
            .map(|i| {
                ...
                let zeta_6 = BigInt::mod_mul(&z_6, &v6[i], &order);
                let zeta_s1 = BigInt::mod_add(&zeta_2, &zeta_3, &order);
                let zeta_s2 = BigInt::mod_add(&zeta_4, &zeta_5, &order);
                let zeta_s3 = BigInt::mod_add(&zeta_s1, &zeta_s2, &order);
                BigInt::mod_add(&zeta_s3, &zeta_6, &order)
            })
            .collect::<Vec<BigInt>>();
        ...
        let nu = (0..t)
            .map(|i| {
                let z2_v7 = BigInt::mod_mul(&z_2, &v7[i], &order);
                let z6_v6 = BigInt::mod_mul(&z_6, &v6[i], &order);
                BigInt::mod_add(&z2_v7, &z6_v6, &order)
            })  
            .collect::<Vec<BigInt>>();
        ...
        let beta = (0..t)
            .map(|i| {
                BigInt::mod_mul(&zeta[i], &theta_inv[i], &order)
        ...
        let z_z5 = BigInt::mod_add(&z, &z_5, &order);
        let one_s_ys_z_plus_z5 = BigInt::mod_mul(&one_s_ys, &z_z5, &order);

        let y_s_z5 = BigInt::mod_mul(&z_5, &y_pow_s, &order);
        ...
        let z6_onet_v6 = BigInt::mod_mul(&z_6, &onet_v6, &order);

        let delta1 = BigInt::mod_add(&z6_onet_v6, &y_s_z5, &order);
        let delta2 = BigInt::mod_add(&delta1, &one_s_ys_z_plus_z5, &order);
        
        let delta: BigInt = (0..t)
            .map(|i| {
                let alp_zeta_i = BigInt::mod_mul(&alpha[i], &zeta[i], &order);
                // BigInt::mod_add(&nu[i], &alp_zeta_i, &order)
                alp_zeta_i
            })
            .fold(delta2.clone(), |acc, x| BigInt::mod_add(&acc, &x, &order));
...
impl MProvePlus {
    pub fn prove(
        ...
        h_vec: &[GE],
        // stmt
        C_vec: &[GE], // vector of commitments
        P_vec: &[GE], // addresses in the ring (public keys)
        H_vec: &[GE], // hash of addresses
        // wit
        E_vec: &[BigInt],
        // a_vec: &[FE], //extra
        // r_vec: &[FE], //extra
        x_vec: &[FE], // secret keys
        gamma: &FE,
    ) -> MProvePlus {
        ...
        let mut I_vec_temp: Vec<GE> = vec![*G; s];
        let mut C_vec_temp: Vec<GE> = vec![*G; s];
        ...
        let e_hat = (0..n)
            .map(|i| {
                ...
                if byte[0]==1 {
                    // construct 'index' numbered row of E_mat
        ...
        let u_square = u.mul(&u.get_element());
        let H_vec_temp = (0..n)
            .map(|i| 
                &H_vec[i] * &u_square
            )
        ...
        let Y_hat_vec = (0..n)
            .map(|i| 
                &H_vec_temp[i] + &P_vec_temp[i]
            )
            .collect::<Vec<GE>>();

        // define compressed I_hat_vec
        let u_bn_sq = BigInt::mod_pow(&u_bn, &BigInt::from(2), &order);
        let minus_u_sq_bn = BigInt::mod_sub(&BigInt::zero(), &u_bn_sq, &order);
        let minus_u_sq: FE = ECScalar::from(&minus_u_sq_bn);
        let I_hat_vec = (0..s)
            .map(|i| {
                let v_i = BigInt::mod_pow(&v_bn, &BigInt::from(i as u32), &order);
                let v_i_fe: FE = ECScalar::from(&v_i);
                let I_vec_u_sq = I_vec[i] * &minus_u_sq;
                I_vec_u_sq * &v_i_fe
            })
            .collect::<Vec<GE>>();
        ...
        let x_inv_vec = (0..s)
            .map(|i|
                ECScalar::invert(&x_vec[i])
            )
            .collect::<Vec<FE>>();

        // convert x_vec and x_vec_inv to big int
        let x_vec_bn = (0..s)
            .map(|i| x_vec[i].to_big_int()).collect::<Vec<BigInt>>();
        let x_inv_vec_bn = (0..s)
            .map(|i| x_inv_vec[i].to_big_int()).collect::<Vec<BigInt>>();

        // define compressed secrets
        let xi = (0..s)
            .map(|i| {
                let v_i = BigInt::mod_pow(&v_bn, &BigInt::from(i as u32), &order);
                let x_u = BigInt::mod_mul(&u_bn, &x_vec[i].to_big_int(), &order);
                BigInt::mod_mul(&v_i, &x_u, &order)
            })
        ...
        let minus_one = BigInt::mod_sub(&BigInt::zero(), &BigInt::one(), &order);
        let mut c_L: Vec<BigInt> = vec![xi.clone()];
        c_L.push(minus_one);
        c_L.push(gamma.to_big_int());
        c_L.extend_from_slice(&e_hat.clone());
        c_L.extend_from_slice(&e_prime.clone());
        c_L.extend_from_slice(&x_inv_vec_bn.clone());
        c_L.extend_from_slice(&E_mat.clone());
        
        // c R := (0^2n+3 || x || vec(E) − 1^sn )
        let mut c_R: Vec<BigInt> = vec![BigInt::zero(); 2*n+3];
        c_R.extend_from_slice(&x_vec_bn.clone());
        ...
        // defining g_w
        let mut g_vec_w: Vec<GE> = vec![*G, C_res, *Gt];
        ...
        let t1 = (0..t)
            .map(|i| {
                ...
                let t1_5 = BigInt::mod_add(&t1_4, &zeta[i], &order);
        ...
        let Rp = (0..t)
            .map(|i| {
                ...
                BigInt::mod_add(&Rp_3, &zeta[i], &order)
            })
            .collect::<Vec<BigInt>>();
        ...
        let rString: &[u8] = &[56, 74, 18, 47];
        let rString_label = BigInt::from(rString);

        let g_vec_append = (0..res)
            .map(|i| {
                let rString_label_i = BigInt::from(i as u32) + &rString_label;
        ...
        let h_vec_append = (0..res)
            .map(|i| {
                let rString_label_i = BigInt::from(n as u32) + BigInt::from(i as u32) + &rString_label;
                let hash_i = HSha256::create_hash(&[&rString_label_i]);
                generate_random_point(&Converter::to_vec(&hash_i))
            })
            .collect::<Vec<GE>>();
    ...
    pub fn verify(
        ...
        let I_hat_vec = (0..s)
            .map(|i| {
                let v_i = BigInt::mod_pow(&v_bn, &BigInt::from(i as u32), &order);
                let v_i_fe: FE = ECScalar::from(&v_i);
                let I_vec_u_sq = self.I_vec[i] * &minus_u_sq;
                I_vec_u_sq * &v_i_fe
            })
            .collect::<Vec<GE>>();
        ...
        // defining g_w
        let mut g_vec_w: Vec<GE> = vec![*G, self.C_res, *Gt];
        g_vec_w.extend_from_slice(&Y_hat_vec);
        g_vec_w.extend_from_slice(&C_vec);
        g_vec_w.extend_from_slice(&I_hat_vec);
        ...
        let verify = self.inner_product_proof.verify(&g_vec, &h_vec_long, &Gx, &P);
...
mod tests {
    ...
    use curv::BigInt;
    use curv::{FE, GE};
    use proofs::mprove_plus::generate_random_point;
    use proofs::mprove_plus::MProvePlus;
    ...
    pub fn test_mprove_plus_aok_1_in_8(){
        ...
        let x_vec = (0..s).map(|_| ECScalar::new_random()).collect::<Vec<FE>>();
        ...
        let mut P_vec = (0..n)
            .map(|i| {
                let label_i = BigInt::from(i as u32) + q_four_hash.clone();
                let hash_i = HSha512::create_hash(&[&label_i]);
                generate_random_point(&Converter::to_vec(&hash_i))
            })
            .collect::<Vec<GE>>();
        ...
        let H_vec = (0..n)
            .map(|i| {
                let label_i = BigInt::from(i as u32) + q_five_hash.clone();
                let hash_i = HSha512::create_hash(&[&label_i]);
                generate_random_point(&Converter::to_vec(&hash_i))
            })
            .collect::<Vec<GE>>();
        ...
        let mprove_test = MProvePlus::prove(&G, &H, &Gt, &H_prime, &p_vec, &g_prime_vec, &h_vec, &C_vec_mut, &P_vec, &H_vec, &E_vec, &x_vec, &gamma);
        let result = mprove_test.verify(&G, &H, &Gt, &H_prime, &p_vec, &g_prime_vec, &h_vec, &C_vec_mut, &P_vec, &H_vec);
        assert!(result.is_ok());
    }
    ...
    pub fn mPlus_simulation_helper(n: usize, s: usize){
    ...
    fn mPlus_simulation(){
        ...
        mPlus_simulation_helper(1000,1);
        // mPlus_simulation_helper(200,20);
        // mPlus_simulation_helper(400,20);
