...
use mProvePlus_ristretto::proofs::omnires::Omnires;
use libc_print::{libc_println};

#[derive(Debug, StructOpt)]
#[structopt(name = "omnires", about = "Omnires proof generation simulator using Ristretto.")]
struct Opt {
...
fn main() {
    // 
    // cargo run --release --bin omnires_ristretto_bin 1000 100 -n 10
    //
    ...
    let (est_G, 
         ...
         est_r_vec,
         est_a_res,
         est_gamma) = Omnires::gen_params(est_n, est_s);
    for _i in 0..5 {        

      gen_proof_start = PreciseTime::now();
      omnires_proof = Omnires::prove(&est_G, 
                                         &est_H, 
                                         &est_H_prime, 
                                         &est_p_vec, 
                                         &est_g_prime_vec, 
      ...
      result = omnires_proof.verify(&est_G, 
                                        &est_H, 
                                        &est_H_prime, 
                                        &est_p_vec, 
                                        &est_g_prime_vec, 
